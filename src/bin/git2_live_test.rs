...
async fn main() {
    ...
    let blob1_hash = node1_vfs
        .create_blob(file1_content)
        .expect("Node 1: Failed to create blob 1");
    println!("Node 1: Created blob 1 with hash: {}", blob1_hash);

    // Create a ref for the main branch
    let main_ref = "refs/heads/main";
    node1_vfs
        .create_ref(main_ref, &blob1_hash)
        .expect("Node 1: Failed to create main ref");
    node1_vfs
        .set_head(main_ref)
        .expect("Node 1: Failed to set HEAD");
    print_vfs_state(&node1_vfs, "Node 1");

    // Create a second commit
    let file2_content = b"Second commit from Node 1.";
    let blob2_hash = node1_vfs
        .create_blob(file2_content)
        .expect("Node 1: Failed to create blob 2");
    node1_vfs
        .update_ref(main_ref, &blob2_hash)
        .expect("Node 1: Failed to update main ref");
    ...
    let node1_main_hash = node1_vfs
        .get_ref(&node1_head_ref)
        .expect("Node 1: Failed to get main ref hash");

    // Simulate fetching the object for the main branch hash
    let object_data = node1_vfs
        .get_object(&node1_main_hash)
        .expect("Node 1: Failed to get object");

    // Node 2 creates the object and ref
    node2_vfs
        .create_object(&node1_main_hash, &object_data)
        .expect("Node 2: Failed to create object");
    node2_vfs
        .create_ref(main_ref, &node1_main_hash)
        .expect("Node 2: Failed to create main ref");
    node2_vfs
        .set_head(main_ref)
        .expect("Node 2: Failed to set HEAD");
    ...
    let feature_blob_hash = node2_vfs
        .create_blob(feature_content)
        .expect("Node 2: Failed to create feature blob");
    node2_vfs
        .create_ref(feature_ref, &feature_blob_hash)
        .expect("Node 2: Failed to create feature ref");
    node2_vfs
        .set_head(feature_ref)
        .expect("Node 2: Failed to set HEAD to feature branch");
    print_vfs_state(&node2_vfs, "Node 2");
    ...
    let node2_feature_hash = node2_vfs
        .get_ref(feature_ref)
        .expect("Node 2: Failed to get feature ref hash");
    let feature_object_data = node2_vfs
        .get_object(&node2_feature_hash)
        .expect("Node 2: Failed to get feature object");

    // Node 1 creates the object and ref for the feature branch
    node1_vfs
        .create_object(&node2_feature_hash, &feature_object_data)
        .expect("Node 1: Failed to create feature object");
    node1_vfs
        .create_ref(feature_ref, &node2_feature_hash)
        .expect("Node 1: Failed to create feature ref");
