...
async fn main() {
    ...
    git_vfs
        .create_ref(branch_name, initial_commit_hash)
        .expect("Failed to create ref");
    println!(
        "Created ref '{}' pointing to: {}",
        branch_name, initial_commit_hash
    );
    ...
    git_vfs
        .update_ref(branch_name, new_commit_hash)
        .expect("Failed to update ref");
    println!("Updated ref '{}' to: {}", branch_name, new_commit_hash);

    // Verify the update
    let updated_main_ref_hash = git_vfs
        .get_ref(branch_name)
        .expect("Failed to get updated ref");
    println!(
        "Verified updated ref '{}' hash: {}",
        branch_name, updated_main_ref_hash
    );
    ...
    println!(
        "SHA256 hash of \"{}\": {}",
        String::from_utf8_lossy(data_to_hash),
        sha256_hash
    );
