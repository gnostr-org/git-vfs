...
pub fn extract_embedded_git_to_temp_dir() -> VfsResult<TempDir> {
    ...
    fn copy_recursively(src: &VfsPath, dest: &Path) -> VfsResult<()> {
        if src.is_dir()? {
            ...
            fs::write(dest, &buffer).map_err(|e| vfs::VfsError::from(e))?;
        }
        Ok(())
    }
